...
const testValidateFile = async () => {
  try {
    ...
    console.log('Total errors count:', result.errors?.length || 0);
    ...
    if (allStudentErrors.length > 0) {
      ...
      console.log('\n🚨 Checking for structural errors that prevent registro 60 validation:');
      ...
      if (structuralErrors.length > 0) {
        ...
        structuralErrors.slice(0, 8).forEach((err, i) => {
          console.log(`${i + 1}. Line ${err.lineNumber} (${err.recordType}): ${err.fieldName} - ${err.ruleName}`);
        });
      }
